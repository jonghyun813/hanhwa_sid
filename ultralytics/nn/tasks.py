...
class BaseModel(torch.nn.Module):
    ...
    def forward(self, x, distillation=False, is_teacher=False, teacher_output=None, teacher_learned_class=None, *args, **kwargs):
        """
        ...
        if isinstance(x, dict):  # for cases of training and validating while training.
            ...
            return self.loss(x, distillation=distillation, is_teacher=is_teacher, teacher_output=teacher_output, teacher_learned_class=teacher_learned_class, *args, **kwargs) # code modify
        # print("DEBUG 3")
    ...
    def _predict_once(self, x, distillation, profile=False, visualize=False, embed=None): # code modify
        """
        Perform a forward pass through the network.
        ...
        for num, m in enumerate(self.model):
            # if num == 0:
            #     temp = m(x)
            #     print(m)
    ...
    def loss(self, batch, distillation=False, is_teacher=False, teacher_output=None, teacher_learned_class=None, preds=None): # code modify
        """
        Compute loss.
        ...
        preds = self.forward(batch["img"]) if preds is None else preds # code modify preds에 gradient 없음
        ...
        if teacher_output: # distillation after pretraining
            ...
            self.criterion = v8DetectionLoss_distillation(self)
