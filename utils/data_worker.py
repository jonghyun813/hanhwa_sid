...
@torch.no_grad()
def worker_loop(index_queue, data_queue, data_dir, transform, transform_on_gpu=False, cpu_transform=None, device='cpu', use_kornia=False, transform_on_worker=True, test_transform=None, scl=False, transform_1 = None, transform_2 = None, transform_3 = None, base_transform=None, normalize = None):
    ...
    while watchdog.is_alive():
        ...
        labels = []
        not_aug_img = []
        transform_1_images = None
        transform_2_images = None
        transform_3_images = None
        if len(r) > 0:
            for sample in r:
                if transform_on_gpu:
                ...
                if transform_on_gpu:
                    if scl:
                        ...
                        if transform_1 is not None:
                            transform_1_images = transform_1(torch.stack(images).to(device))
                            transform_2_images = transform_2(transform_1_images)
                            transform_3_images = transform_3(transform_2_images)
                            not_aug_images = base_transform(torch.stack(images).to(device))
            ...
            if transform_1_images is not None:
                data['transform_1_image'] = normalize(transform_1_images)
                data['transform_2_image'] = normalize(transform_2_images)
                data['transform_3_image'] = normalize(transform_3_images)
                data['not_aug_image'] = not_aug_images
